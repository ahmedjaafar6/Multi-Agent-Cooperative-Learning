...
def run_episode(epsilon1, epsilon2, alpha, gamma1, gamma2):
    ...
    while True:
        ...
        #agent 1 q updates
        max_a_prime_indx = np.argmax(q_hat1[agent1[0], agent1[1]]) #max action index

        curr_q = q_hat1[curr_y1, curr_x1, a_map[action1]]
        next_q = q_hat1[agent1[0], agent1[1], max_a_prime_indx]
        ...
        #agent 2 q updates
        max_a_prime_indx = np.argmax(q_hat2[agent2[0], agent2[1]]) #max action index

        curr_q = q_hat2[curr_y2, curr_x2, a_map[action2]]
        next_q = q_hat2[agent2[0], agent2[1] , max_a_prime_indx]
